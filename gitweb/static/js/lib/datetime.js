...
function timezoneOffset(timezoneInfo) {
  const match = tzRe.exec(timezoneInfo);
  const tz_sign = match[1] === "-" ? -1 : +1;
  const tz_hour = parseInt(match[2], 10);
  const tz_min = parseInt(match[3], 10);
...
function formatDateISOLocal(epoch, timezoneInfo) {
  ...
  const localDate = new Date(1000 * (epoch + timezoneOffset(timezoneInfo)));
  const localDateStr = // e.g. '2005-08-07'
    localDate.getUTCFullYear() +
    "-" +
    padLeft(localDate.getUTCMonth() + 1, 2, "0") +
    "-" +
    padLeft(localDate.getUTCDate(), 2, "0");
  const localTimeStr = // e.g. '21:49:46'
...
function formatDateRFC2882(epoch, timezoneInfo, padDay) {
  ...
  // date corrected by timezone
  const localDate = new Date(1000 * (epoch + timezoneOffset(timezoneInfo)));
  const localDateStr = // e.g. 'Sun, 7 Aug 2005' or 'Sun, 07 Aug 2005'
