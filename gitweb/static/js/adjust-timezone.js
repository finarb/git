...
function fixDatetimeTZ(tz, tzClassName, nochange) {
  ...
  for (let i = 0, len = classesFound.length; i < len; i++) {
    ...
    if (!nochange) {
      ...
      const adjusted = formatDateRFC2882(epoch, tz);
...
function addChangeTZ(tzSelected, tzCookieInfo, tzClassName) {
  ...
  let tzSelectFragment = document.createDocumentFragment();
...
function removeChangeTZForm(documentFragment, target, tzClassName) {
  ...
  const removed = popup.parentNode.removeChild(popup);
...
function generateTZList() {
  ...
  for (let x = -12, idx = timezones.length; x <= +14; x++, idx++) {
    const hours = (x >= 0 ? "+" : "-") + padLeft(x >= 0 ? x : -x, 2);
...
function generateTZOptions(tzSelected) {
  ...
  const timezones = generateTZList();

  for (let i = 0, len = timezones.length; i < len; i++) {
    const tzone = timezones[i];
...
function selectTZHandler(tzSelectFragment, tzCookieInfo, tzClassName) {
  ...
  return function (event) {
    ...
    const selected = target.options.item(target.selectedIndex);
